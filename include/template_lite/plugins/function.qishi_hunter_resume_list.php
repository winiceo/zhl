<?php
function tpl_function_qishi_hunter_resume_list($params, &$smarty)
{
global $db,$_CFG;
...
if (!empty($aset['age']))
{
	$age_sql = "select c_name from ".table('category')." where c_id=".$aset['age'];
	$age_res =  $db->getone($age_sql);
	
	$age1 = date("Y")-(substr($age_res['c_name'],'0','2'));
	$age2 = date("Y")-(substr($age_res['c_name'],'3','2'));
	
	$wheresql = !empty($wheresql)?($wheresql." and birthdate between ".$age2." and ".$age1):(" r.birthdate between ".$age2." and ".$age1);
}
