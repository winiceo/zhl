...
if ($act == 'list') {
    ...
    write_memberslog(intval($_POST['personal_uid']), 2, 9001, $user['username'], " 管理员操作葫芦币({$t}{$_POST['points']})，(剩余:{$points})，备注：" . $_POST['points_notes'], 2, 1012, "管理员操作葫芦币", "{$t}{$_POST['points']}", "{$points}");
    ...
    $log_list = get_resume_check_loglist($resume["tmp_id"]);
...
if ($act == 'list_temp') {
    ...
    if ($cate == 1) {
        ...
        foreach($resumelist as $key=>$val){
            $resumelist[$key]["check_percent"]=get_pass_percent($val["id"]);
        }
    } elseif ($cate == 2) {
        ...
        $resumelist = get_resume_pass_list($offset, $perpage, $getsql);

        foreach($resumelist as $key=>$val){
            $resumelist[$key]["check_percent"]=100;
        }
    }
    ...
    if (!empty($_REQUEST['delete'])) {
        ...
        if ($n = del_resume_temp($id)) {
            ...
            adminmsg("删除成功！该删除 {$n} 行", 2, $link);
        } else {
            adminmsg("删除失败！", 0, $link);
        }
    }
    if (!empty($_REQUEST['set_audit'])) {
        $rs=resume_check_log($id, $pass_result);
        if($rs){
            $result = import_resume_temp($id);
        }else{
            $result["msg"]="拒绝成功";
        }
       ...
       // resume_check_log($id[0], $pass_result);
...
//添加浏览记录
function resume_check_log($id, $result = "")
{
...
function get_pass_percent($id){
    $resume = ORM::for_table(table("resume_check_log"))->where("rid", $id)->where("pass",1)->count("*");


    return intval($resume/3*100);
