...
<input type="radio" name="closetime" value="1"  <?php if ($this->_vars['config']['closetime'] == "1"): ?>checked=checked<?php endif; ?>/>是</label></td>
    ...
    <tr>
      ...
      <td>
      <label>
      <input name="check_reg_email" type="radio" id="check_reg_email" value="0"  <?php if ($this->_vars['config']['check_reg_email'] == 0): ?>checked=checked <?php endif; ?>/>否</label>
      &nbsp;&nbsp;&nbsp;&nbsp;
      <label >
      <input type="radio" name="check_reg_email" value="1"  <?php if ($this->_vars['config']['check_reg_email'] == "1"): ?>checked=checked<?php endif; ?>/>是</label></td>
    </tr>

    <tr>
      <td align="right">网站主体背景：</td>
      <td><input name="body_bgimg" type="file" id="body_bgimg" style="width:170px; font-size:12px; padding:3px;" onKeyDown="alert('请点击右侧“浏览”选择您电脑上的图片！');return false"/>&nbsp;<?php if ($this->_vars['config']['body_bgimg']): ?><input type="checkbox" name="set_body_bgimg_defaule" value="1">设置网站主体背景为默认<?php endif; ?>&nbsp;&nbsp;(背景图片尺寸：1000x156 像素)<br><input type="button" name="Submit" value="查看网站主体背景" class="vtip" title="<img src=<?php if ($this->_vars['config']['body_bgimg']):  echo $this->_vars['QISHI']['upfiles_dir'];  echo $this->_vars['config']['body_bgimg'];  else:  echo $this->_vars['QISHI']['site_dir']; ?>
